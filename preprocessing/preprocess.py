...
def run_preprocessing(config: dict):
    ...
    1）多样化-模拟信号的第5组和第3组使用魔术' group_0 + group_0 ~ group_5 '和'
     group_4 + group_4 ~ group_3 '。创建大型和多样化的数据集，
     这有助于模型的泛化和给出比训练原始数据更好的分数。

   2) OVERFIT -生成死亡数据集。仍然使用' group_4 + group_4 ~ group_3 '魔法，
   但只选择由combinatorial_synthesis()产生的高度相关片段，并对特定信号造成噩梦般的过拟合
   模式。你的模型越复杂，后果就越严重。GroupKFold简历会撒谎，把你带向错误的方向。

   3) REDUCED -特殊模式，采用信道缩减技术。可以用来利用漏洞。
    :return:
    """
    # Reading config
